        ...
        <div style={{ display:'flex', gap:8, overflowX:'auto', paddingBottom:4 }}>
          {imgs.map((url, i) => (
            <div key={i} onClick={() => setActiveIdx(i)}
              style={{ width:88, height:60, flexShrink:0, borderRadius:8, overflow:'hidden', cursor:'pointer', border:`2px solid ${i===activeIdx?'#D4420A':'transparent'}`, transition:'border .15s', background:'#ddd' }}>
...
function InfoRow({ label, value, money, tag }) {
  ...
  return (
    <div style={{ display:'flex', padding:'7px 16px', gap:12, borderBottom:'1px solid #f5f3f0' }}>
      <div style={{ fontSize:12, color:'#909090', minWidth:140, flexShrink:0, paddingTop:1 }}>{label}</div>
      <div style={{ fontSize:12.5, fontWeight:500, color: money ? '#D4420A' : '#1C1C1C', flex:1, wordBreak:'break-word' }}>
...
export default function XeDetail() {
  ...
  return (
    <div style={{ background:'#F5F3EF', minHeight:'100vh', fontFamily:"'Be Vietnam Pro',sans-serif" }}>
      {/* Topbar */}
      <div style={{ background:'#1A1A1A', height:52, display:'flex', alignItems:'center', padding:'0 24px', gap:14, position:'sticky', top:0, zIndex:100 }}>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'28px 24px', display:'grid', gridTemplateColumns:'1fr 420px', gap:32, alignItems:'start' }}>
        ...
        <div>
          <div style={{ marginBottom:18 }}>
            <div style={{ fontSize:26, fontWeight:700, letterSpacing:.5, lineHeight:1.2 }}>{xe.bienSo}</div>
      ...
      {/* History — full width */}
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 24px 32px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
