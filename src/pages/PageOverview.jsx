  ...
  const handleFile = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        // Find header row (has "Biển số" or "STT")
        let headerIdx = -1
        for (let i = 0; i < Math.min(5, allRows.length); i++) {
          const joined = allRows[i].join(',')
          if (/biển số|bien so|STT/i.test(joined)) { headerIdx = i; break }
        }
        if (headerIdx < 0) { setError('Không tìm thấy dòng tiêu đề'); return }
        ...
        if (!rows.length) { setError('Không đọc được dữ liệu'); return }

        // Identify key columns
        const keyTT   = headers.find(h => /tình trạng|tinh trang/i.test(h)) || ''
        const keyBS   = headers.find(h => /biển số|bien so/i.test(h)) || ''
        ...
        const keyNam  = headers.find(h => /năm sản xuất|nam sx/i.test(h)) || ''

        const classified = rows.map(r => {
          const tt = String(r[keyTT] || '').trim()
          // Exact match first, then fuzzy
          let type = 'hoat_dong'
          if (tt === 'Dừng hoạt động' || /^dừng/i.test(tt)) type = 'dung'
          else if (tt === 'Hoạt động kém hiệu quả' || /kém|kem hiệu/i.test(tt)) type = 'kem'
          const kmRaw = String(r[keyKm] || '').replace(/,/g,'')
          return {
            ...r,
            _bienSo:  r[keyBS],
            _type:    type,
        ...
        setData({ rows: classified, dung, kem, hoat_dong: hd,
          headers, keyTT, keyBS, keyCH, keyTinh, keyKm, keyNgay, keyLoai, keyNam,
          fileName: file.name })
        setTab('all')
      } catch(err) {
              ...
              <tbody>
                {rowsToShow.map((row, i) => (
                  <tr key={i} style={{ background: i%2===0 ? '#FAFAFA' : '#fff' }}>
                    ...
                    <td style={{ padding:'8px 14px', textAlign:'center', color:'#374151', borderBottom:'1px solid #F3F4F6', maxWidth:180, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{row._loaiXe || '—'}</td>
