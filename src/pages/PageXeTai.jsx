...
export default function PageXeTai({ data, rowsLoaded }) {
  ...
  const visibleCols = ALL_COLS.filter(c => visibleKeys.includes(c.k))
  ...
  const filtered = useMemo(() => {
    ...
    return r
  }, [rows, search, filterMien, filterLT, sortCol, sortDir, visibleCols])
    ...
    <div>
                    ...
                    <tr key={r.maTaiSan || idx} style={{ borderBottom:'1px solid var(--border)' }}>
                      {visibleCols.map(col => {
                        ...
                        if (col.k === 'mien') {
                          ...
                          return <td key="mien" style={{ padding:'8px 10px' }}><span style={{ background:s2.bg, color:s2.color, fontWeight:600, fontSize:10.5, padding:'2px 7px', borderRadius:4 }}>{r.mien}</span></td>
                        }
                        if (col.k === 'gtcl' || col.k === 'nguyenGia') return <td key={col.k} style={{ padding:'8px 10px', textAlign:'right' }}>{r[col.k] ? fmtCur(r[col.k]) : '—'}</td>
                        if (col.k === 'taiTrong') return <td key="tt" style={{ padding:'8px 10px', textAlign:'right' }}>{r.taiTrong}</td>
                        if (col.k === 'hasTaiNan') return <td key="tn" style={{ padding:'8px 10px' }}>{r.hasTaiNan ? <span style={{ background:'var(--red-l)', color:'var(--red)', fontWeight:600, fontSize:10.5, padding:'2px 7px', borderRadius:4 }}>Có</span> : ''}</td>
                        if (col.k === 'hasDieuDong') return <td key="dd" style={{ padding:'8px 10px' }}>{r.hasDieuDong ? <span style={{ background:'var(--teal-l)', color:'var(--teal)', fontWeight:600, fontSize:10.5, padding:'2px 7px', borderRadius:4 }}>Có</span> : ''}</td>
      ...
      {showColPicker && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.4)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center' }}
          ...
          <div style={{ background:'#fff', borderRadius:14, width:480, maxWidth:'95vw', boxShadow:'0 8px 40px rgba(0,0,0,.18)', overflow:'hidden' }}
                    ...
                    <input type="checkbox" checked={checked} disabled={fixed}
                      onChange={() => {
                        ...
                        setVisibleKeys(prev => {
                          ...
                          return ALL_COLS.map(c => c.k).filter(k => next.includes(k))
                        })
                      }}
                      style={{ display:'none' }}
                    />
