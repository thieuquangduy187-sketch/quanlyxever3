    ...
    <div>
      <div style={{ display:'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap:12, marginBottom:16 }}>
        {[
          ...
          <div key={i} style={{ background:'var(--bg-card)', border:'0.5px solid var(--sep)', borderRadius:12, padding:'16px 18px', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:12, marginBottom:12 }}>
        <div style={{ background:'var(--bg-card)', border:'0.5px solid var(--sep)', borderRadius:12, padding:'15px 18px' }}>
          <div style={{ fontSize:12.5, fontWeight:600, color:'var(--label-primary)', marginBottom:8 }}>Theo nhãn hiệu</div>
          ...
          {nhArr.slice(0,6).map((d, i) => (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:6, marginTop:5, fontSize:11.5 }}>
              ...
              <b>{d.value}</b>
              <span style={{ color:'var(--label-secondary)', minWidth:30 }}>{s.total ? Math.round(d.value/s.total*100) : 0}%</span>
            </div>
          ))}
        </div>
        <div style={{ background:'var(--bg-card)', border:'0.5px solid var(--sep)', borderRadius:12, padding:'15px 18px' }}>
          <div style={{ fontSize:12.5, fontWeight:600, color:'var(--label-primary)', marginBottom:12 }}>Theo đơn vị sử dụng</div>
          {dvArr.slice(0,12).map((d, i) => {
            const max = Math.max(...dvArr.map(x => x.value), 1)
            return (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
                <div style={{ fontSize:12, color:'var(--label-primary)', minWidth:80, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{d.name}</div>
      ...
      {rows.length > 0 && (
        <div style={{ background:'var(--bg-card)', border:'0.5px solid var(--sep)', borderRadius:12, overflow:'hidden' }}>
          <div style={{ padding:'15px 18px 10px', fontSize:12.5, fontWeight:600, color:'var(--label-primary)' }}>Danh sách ô tô con</div>
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
              <thead>
                <tr>
                  {['STT','Biển số','Nhãn hiệu','Đơn vị SD','Pháp nhân','Số chỗ','Năm SX','GTCL','Nhân sự'].map(h => (
                    <th key={h} style={{ padding:'9px 12px', background:'var(--fill-tertiary)', borderBottom:'1px solid var(--border)', fontWeight:600, fontSize:10.5, color:'var(--label-secondary)', textTransform:'uppercase', letterSpacing:'.05em', whiteSpace:'nowrap', textAlign:'left' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
