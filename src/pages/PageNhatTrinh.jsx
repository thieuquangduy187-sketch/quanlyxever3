...
export default function PageNhatTrinh({ user }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/gia-dau?thang=${form.thang}&nam=${form.nam}`, {
      ...
      .then(r => r.json())
      .then(d => { if (d && d.available) setPriceData(d) })
      .catch(() => {})
  }, [form.thang, form.nam])
  ...
  function validate(f = form) {
    ...
    // // Cảnh báo giá dầu — priceData từ state (fetch tự động từ /api/gia-dau)
    const litDau  = Number(f.tongLitDau)  || 0
    const tienDau = Number(f.tongTienDau) || 0
    if (litDau > 0 && tienDau > 0 && priceData) {
      const donGia = Math.round(tienDau / litDau)
      const soKy   = priceData.soLanDieuChinh || '?'
      if (donGia < priceData.min * 0.85) {
        errs.tongTienDau = `Đơn giá ${donGia.toLocaleString('vi-VN')}đ/lít thấp bất thường. Bình quân T${f.thang}/${f.nam} (${soKy} kỳ điều chỉnh): ${priceData.avg.toLocaleString('vi-VN')}đ | Thấp nhất: ${priceData.min.toLocaleString('vi-VN')}đ. Kiểm tra lại.`
      } else if (donGia > priceData.max * 1.15) {
        errs.tongTienDau = `Đơn giá ${donGia.toLocaleString('vi-VN')}đ/lít cao bất thường. Bình quân T${f.thang}/${f.nam} (${soKy} kỳ điều chỉnh): ${priceData.avg.toLocaleString('vi-VN')}đ | Cao nhất: ${priceData.max.toLocaleString('vi-VN')}đ. Kiểm tra lại.`
      }
    }
    return errs
  }
  ...
  async function handleSubmit() {
    ...
    // Check required
    const required = ['kmDauThang', 'kmCuoiThang', 'tongLitDau', 'tongKLChuyen', 'soChuyenXe']
          ...
          <div key={groupKey} style={card}>
            ...
            <div style={{ ...cardBody, display:'grid', gridTemplateColumns: fields.length >= 3 ? '1fr 1fr 1fr' : fields.length === 2 ? '1fr 1fr' : '1fr', gap:12 }}>
              {fields.map(f => {
                ...
                const isRequired = ['kmDauThang','kmCuoiThang','tongLitDau','tongKLChuyen','soChuyenXe'].includes(f.key)
                ...
                return (
                  <div key={f.key}>
                    ...
                    {/* Gợi ý giá dầu cho trường tongTienDau */}
                                        {f.key === 'tongTienDau' && !hasErr && priceData && (
                      <div style={{ fontSize:11, color:'var(--label-tertiary)', marginTop:4, lineHeight:1.8 }}>
                        📊 T{form.thang}/{form.nam} · {priceData.soLanDieuChinh || '?'} kỳ điều chỉnh · Nguồn: Petrolimex
                        {priceData.do001?.v1 && <span style={{marginLeft:8}}>
                          DO 0,001S-V: <strong style={{color:'var(--apple-blue)'}}>{priceData.do001.v1.toLocaleString('vi-VN')}đ</strong> (V1) / {priceData.do001.v2?.toLocaleString('vi-VN')}đ (V2)
                        </span>}
                        {priceData.do05?.v1 && <span style={{marginLeft:8}}>
                          DO 0,05S-II: <strong style={{color:'var(--apple-green)'}}>{priceData.do05.v1.toLocaleString('vi-VN')}đ</strong> (V1) / {priceData.do05.v2?.toLocaleString('vi-VN')}đ (V2)
                        </span>}
                        <span style={{marginLeft:8, color:'var(--label-quaternary)'}}>
                          (min {priceData.min?.toLocaleString('vi-VN')} – max {priceData.max?.toLocaleString('vi-VN')}đ)
                        </span>
                      </div>
                    )}                 )}                  </div>
                )
