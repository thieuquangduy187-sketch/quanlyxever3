...
export default function PageNhatTrinh({ user }) {
  ...
  const [editingRecord,  setEditingRecord]  = useState(null)  // null = tạo mới, object = đang sửa
  ...
  // Nguồn: giaxanghomnay.com + moit.gov.vn (Petrolimex, đồng/lít, vùng 1)
  // DO 0.001S-V bình quân tất cả kỳ điều chỉnh trong tháng
  // Biển số đang chọn (khi user có nhiều xe)
  const bienSoList = user?.bienSoList || []
  const hasMultiBienSo = bienSoList.length > 1

  const PRICE_FALLBACK = {
    // T1/2026: ~3 kỳ, giá ổn định thấp
    ...
    // T3/2026: 10 kỳ (5/3,7/3,10/3,11/3,12/3,19/3,24/3,25/3,26/3,27/3)
    // 5/3: giá tăng mạnh (~26.900đ), 10-12/3 hạ còn 27.025đ do quỹ bình ổn, 24/3 đỉnh 39.860đ
    '3/2026': { available:true, min:26900, max:39860, avg:32144, soLanDieuChinh:10,
      do001:{v1:32144,v2:32787}, do05:{v1:30778,v2:31394} },
    // T4/2026: 6 kỳ (2/4,3/4,8/4,9/4,16/4,21/4), đỉnh 44980 ngày 3/4
    // T4/2026: 7 kỳ — cập nhật 23/4 (15h) giảm còn 26.697đ
    '4/2026': { available:true, min:26697, max:44980, avg:35448, soLanDieuChinh:7,
      do001:{v1:35868,v2:36585}, do05:{v1:34326,v2:35013} },
  }
  ...
  // Load record vào form để chỉnh sửa
  function handleEdit(record) {
    setSelectedRecord(null)
    setForm({
      thang:        record.thang,
      nam:          record.nam,
      kmDauThang:   String(record.kmDauThang   || ''),
      kmCuoiThang:  String(record.kmCuoiThang  || ''),
      kmDuongDeo:   String(record.kmDuongDeo   || ''),
      tgSuDungCau:  String(record.tgSuDungCau  || ''),
      tongLitDau:   String(record.tongLitDau   || ''),
      tongTienDau:  String(record.tongTienDau  || ''),
      tongKLChuyen: String(record.tongKLChuyen || ''),
      klNoiBo:      String(record.klNoiBo      || ''),
      soChuyenXe:   String(record.soChuyenXe   || ''),
      cpThueNgoai:  String(record.cpThueNgoai  || ''),
      klThueNgoai:  String(record.klThueNgoai  || ''),
      ghiChu:       record.ghiChu || '',
      bienSoChon:   record.bienSo || '',
    })
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const url    = isEditing ? `${API}/api/nhat-trinh/${editingRecord._id}` : `${API}/api/nhat-trinh`
      const method = isEditing ? 'PUT' : 'POST'
      const res = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${getToken()}` },
        body: JSON.stringify({ ...form, bienSoChon: form.bienSoChon || bienSoList[0] || user?.bienSo })
      })
  ...
  const cardBody = { padding: isMobile ? '12px 14px' : '16px 20px' }

  const inputWrap = { position:'relative', display:'flex', alignItems:'center' }
  const inp = (hasError) => ({
    width:'100%',
    padding: isMobile ? '14px 14px' : '11px 14px',
    paddingRight: 52,
  ...
  return (
    <div style={{ maxWidth:680, margin:'0 auto', padding: isMobile ? '0 0 80px' : '0 0 40px' }}>
      ...
      <div style={card}>
        ...
        <div style={{ ...cardBody, display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: isMobile ? 10 : 12 }}>
          ...
          {/* Dropdown chọn biển số — chỉ hiện khi user có nhiều xe */}
          {hasMultiBienSo && (
            <div>
              <label style={{ fontSize:12, fontWeight:500, color:'var(--label-secondary)', marginBottom:4, display:'block' }}>
                Chọn biển số xe <span style={{ color:'var(--apple-red)' }}>*</span>
              </label>
              <select
                value={form.bienSoChon}
                onChange={e => setForm(f => ({...f, bienSoChon: e.target.value}))}
                style={{
                  width:'100%', padding:'11px 14px', boxSizing:'border-box',
                  background: form.bienSoChon ? '#1C1C1E' : '#3a1a1a',
                  border: `0.5px solid ${form.bienSoChon ? '#3a3a3c' : 'var(--apple-red)'}`,
      ...
      {Object.entries(byGroup).map(([groupKey, fields]) => {
        ...
        return (
          <div key={groupKey} style={card}>
            ...
            <div style={{ ...cardBody, display:'grid', gridTemplateColumns: isMobile ? '1fr' : fields.length >= 3 ? '1fr 1fr 1fr' : fields.length === 2 ? '1fr 1fr' : '1fr', gap: isMobile ? 10 : 12 }}>
              {fields.map(f => {
      ...
      {editingRecord && (
        <div style={{
          background: '#FF9F0A22', border: '0.5px solid #FF9F0A',
          borderRadius: 12, padding: '10px 16px', marginBottom: 12,
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#FF9F0A' }}>
              ✏️ Đang chỉnh sửa T{editingRecord.thang}/{editingRecord.nam}
            </div>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', marginTop: 2 }}>
              Dữ liệu mới sẽ ghi đè lên bản nộp trước đó
            </div>
          </div>
          <button onClick={handleCancelEdit}
            style={{ background: 'none', border: '0.5px solid rgba(255,255,255,0.2)',
      ...
      <button onClick={handleSubmit} disabled={loading || Object.keys(errors).filter(k=>k!=='_form').length>0}
        style={{
          width:'100%', padding: isMobile ? '18px' : '15px', borderRadius:14, border:'none',
          background: loading ? 'rgba(0,122,255,0.5)' : 'var(--apple-blue)',
          color:'#fff', fontSize: isMobile ? 17 : 16, fontWeight:700,
      ...
      >
        ...
        ) : editingRecord ? '✓ Xác nhận điều chỉnh' : '📤 Nộp nhật trình'}
      </button>
            ...
            <div key={i}
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.45)', marginTop:3,
                  ...
                  {h.tongKLChuyen > 0 && <span>📦 {h.tongKLChuyen?.toLocaleString('vi-VN')} kg</span>}
                  {h.tgSuDungCau > 0 && <span>⏱ {h.tgSuDungCau} phút cẩu</span>}
                </div>
              </div>
      ...
      {/* Modal xem chi tiết nhật trình đã nộp */}
      {selectedRecord && (
        <div style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            padding:'0 0 20px', maxHeight:'88vh', overflowY:'auto',
          }}>
            {/* Handle bar */}
            ...
            {/* Header */}
            <div style={{ padding:'12px 20px 14px', borderBottom:'0.5px solid var(--sep)',
              display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div>
                <div style={{ fontSize:18, fontWeight:700, color:'rgba(255,255,255,0.88)' }}>
                  Nhật trình T{selectedRecord.thang}/{selectedRecord.nam}
                </div>
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.4)', marginTop:2 }}>
                  Biển số: {selectedRecord.bienSo || '—'} · Nộp: {selectedRecord.submittedAt
                    ? new Date(selectedRecord.submittedAt).toLocaleDateString('vi-VN')
                    : '—'}
                </div>
              </div>
              <button onClick={() => setSelectedRecord(null)}
                style={{ background:'rgba(255,255,255,0.1)', border:'none', color:'rgba(255,255,255,0.6)',
                  borderRadius:20, width:32, height:32, fontSize:16, cursor:'pointer', fontFamily:'inherit' }}>
                ✕
              </button>
            </div>
            {/* Stats grid */}
            <div style={{ padding:'16px 20px', display:'grid',
              gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 1fr', gap:8 }}>
              {[
                { lbl:'Km đầu tháng',   val: selectedRecord.kmDauThang?.toLocaleString('vi-VN') || '—', unit:'km' },
                { lbl:'Km cuối tháng',  val: selectedRecord.kmCuoiThang?.toLocaleString('vi-VN') || '—', unit:'km' },
                { lbl:'Tổng km',        val: selectedRecord.tongKmDiChuyen?.toLocaleString('vi-VN') || '—', unit:'km', color:'#0A84FF' },
                { lbl:'Km đường đèo',   val: selectedRecord.kmDuongDeo?.toLocaleString('vi-VN') || '0', unit:'km' },
                { lbl:'Số chuyến xe',   val: selectedRecord.soChuyenXe || '—', unit:'chuyến' },
                { lbl:'Thời gian cẩu',  val: selectedRecord.tgSuDungCau || '0', unit:'phút', color:'#FF9F0A' },
                { lbl:'Tổng lít dầu',   val: selectedRecord.tongLitDau?.toLocaleString('vi-VN') || '—', unit:'lít' },
                { lbl:'Tổng tiền dầu',  val: selectedRecord.tongTienDau ? (selectedRecord.tongTienDau/1e6).toFixed(1)+'M' : '—', color:'#FF453A' },
                { lbl:'Tổng KL chuyên chở', val: selectedRecord.tongKLChuyen?.toLocaleString('vi-VN') || '—', unit:'kg', color:'#30D158' },
                { lbl:'KL nội bộ',      val: selectedRecord.klNoiBo?.toLocaleString('vi-VN') || '0', unit:'kg' },
                { lbl:'CP thuê ngoài',  val: selectedRecord.cpThueNgoai ? (selectedRecord.cpThueNgoai/1e6).toFixed(1)+'M' : '0' },
                { lbl:'KL thuê ngoài',  val: selectedRecord.klThueNgoai?.toLocaleString('vi-VN') || '0', unit:'kg' },
              ].map(s => (
                <div key={s.lbl} style={{ background:'#111', borderRadius:10, padding:'10px 12px' }}>
                  <div style={{ fontSize:10, color:'rgba(255,255,255,0.3)', marginBottom:4,
                    textTransform:'uppercase', letterSpacing:'0.3px' }}>{s.lbl}</div>
                  <div style={{ fontSize:16, fontWeight:700, color: s.color || 'rgba(255,255,255,0.88)' }}>
                    {s.val}{s.unit && <span style={{ fontSize:11, fontWeight:400, marginLeft:3,
                      color:'rgba(255,255,255,0.4)' }}>{s.unit}</span>}
                  </div>
                </div>
              ))}
            </div>
            {selectedRecord.ghiChu && (
              <div style={{ margin:'0 20px 12px', padding:'12px', background:'#111',
                borderRadius:10, border:'0.5px solid var(--sep)' }}>
                <div style={{ fontSize:10, color:'rgba(255,255,255,0.3)', marginBottom:4,
                  textTransform:'uppercase', letterSpacing:'0.3px' }}>Ghi chú</div>
            ...
            <div style={{ padding:'0 20px' }}>
              <button
                ...
                style={{
                  width:'100%', padding: isMobile ? '15px' : '13px',
                  borderRadius:12, border:'none',
                  background:'#0A84FF',
                  color:'#fff', fontSize:15, fontWeight:700,
                  cursor:'pointer', fontFamily:'inherit',
                }}>
                ✏️ Chỉnh sửa nhật trình này
              </button>
            </div>
          </div>
        </div>
