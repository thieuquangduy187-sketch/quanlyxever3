...
export default function App() {
  const [page, setPage] = useState(() => {
    try {
      ...
      if (u?.role === 'xe') return 'nhat_trinh_ngay'
      // Admin/viewer: restore last page but never nhat_trinh
      const saved = sessionStorage.getItem('hsg_page')
      if (saved && !['nhat_trinh','nhat_trinh_ngay'].includes(saved)) return saved
      return 'overview'
  ...
  const handleNavChange = useCallback((newPage) => {
    ...
    if (user?.role === 'xe' && !['nhat_trinh','nhat_trinh_ngay'].includes(newPage)) return
    setPage(newPage)
  ...
  if (!user || !token) return <LoginScreen onLogin={(u, t) => {
    ...
    if (u?.role === 'xe') {
      setPage('nhat_trinh_ngay')
      sessionStorage.setItem('hsg_page', 'nhat_trinh_ngay')
    } else {
      setPage('overview')
