...
export default function App() {
  const [page, setPage] = useState(() => {
    // Xe users always go to nhat_trinh
    try {
      const u = JSON.parse(localStorage.getItem('hsg_user'))
      if (u?.role === 'xe') return 'nhat_trinh'
      // Other users: restore last page from session
      return sessionStorage.getItem('hsg_page') || 'overview'
  ...
  if (!user || !token) return <LoginScreen onLogin={(u, t) => {
    ...
    if (u?.role === 'xe') { setPage('nhat_trinh'); sessionStorage.setItem('hsg_page','nhat_trinh') }
  }} />
