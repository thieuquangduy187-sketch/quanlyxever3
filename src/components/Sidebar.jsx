...
export default function Sidebar({ page, onNav, data, refreshing, onRefresh, lastUpdated, loadProgress = {} }) {
  return (
    <aside style={{
        ...
        {NAV.map(item => {
          ...
          return (
            ...
            >
              ...
              {/* Loading spinner when fetching rows */}
              {status === 'loading' && (
      ...
      <div style={{ padding:'10px 8px 14px', borderTop:'1px solid rgba(255,255,255,.07)' }}>
        <button onClick={onRefresh} disabled={refreshing}
          style={{ display:'flex', alignItems:'center', gap:7, width:'100%', padding:'9px 11px', borderRadius:8, background:'rgba(255,255,255,.05)', border:'none', cursor: refreshing ? 'not-allowed' : 'pointer', color:'rgba(255,255,255,.45)', fontSize:12, fontFamily:'inherit', opacity: refreshing ? .5 : 1 }}
        >
